...
<table width="100%" class="m-dt-table table table-bordered table-hover" id="html_table">
    <thead class="m-datatable__head">
        ...
        <th>{{trans('sidebar.collar_sleeve_style')}} {{trans('user.title')}} </th>
        <th>{{trans('common.image')}} </th>
        <th>{{trans('sidebar.kandora_style')}} </th>
    ...
    <tbody class="m-datatable__body">
        @forelse($lists as $row) 
            <tr class="m-datatable__row">
                ...
                <td>{{$row->kandoraStyle->title}}</td>
                <td><a href="{{ URL::to('admin/collar-style/'.$row->id.'/edit') }}"><i class="fa fa-pencil"></i></a> &nbsp; <a href="#" class="delete" data-id="{{$row->id}}"> <i class="fa fa-times-circle-o"></i></a></td>                                          
            </tr>
        @empty
        <tr class="odd gradeX">
